...
| 7 | 后挡风轮廓 + 透光区 | `ADVANCED_FACE` | 命名面 | ⏳ 需补 `后挡风`/`透光区` |
...
- **建议额外命名镜片面** `镜片`(旧名 `INNER_MIRROR_GLASS`):镜座总成内有 34 个平面,系统需从中认出镜片(取最大平面)。若供应商直接命名镜片面,识别更稳。
- 系统提取:圈出 `内后视镜镜座`(或 `镜片`)总成 → 找最大平面面 → 追踪边界得镜面轮廓(2D u-v)→ 由轮廓跨度导出 width/height/corner_radius(对齐现有 step_topology 提取逻辑)。
...
- 优先 `眼点左`/`眼点右`(modena 旧名 `左侧眼椭圆中心点`/`右侧眼椭圆中心点`,已命名 ✅);兜底 `眼椭圆`(眼中心)。
...
- `透光区`(旧名 `REAR_WINDOW_TZ`):后挡风透光区面/曲线(命名)。系统得 4 点透光区。
...
- [ ] (建议)镜片面命名 `镜片`(旧名 `INNER_MIRROR_GLASS`)
- [ ] 含命名点 `球铰`(球铰中心;旧名 `MIRROR_PIVOT`)
- [ ] 含命名点 `镜心`(镜面零位中心;旧名 `MIRROR_CENTER_ZERO`)
- [ ] 含命名点 `眼点左` / `眼点右`(或 modena 已有 `眼椭圆` / `左右眼椭圆中心点`)
- [ ] 含命名点 `地面前` / `地面后`(或 `curb0 ground line` 曲线)
- [ ] 含命名面 `后挡风` + `透光区`(后挡风外框 + 透光区)
