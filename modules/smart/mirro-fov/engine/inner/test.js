...
// ======================================================================
// 1. Modena 默认角度五线校核 (对照 Python: 5/5 PASS, 命中点逐位一致)
// ======================================================================
console.log("=== 1. Modena 默认角度五线校核 ===\n");
const modena = loadVehicle('modena');
const m = new Mirror({
  ...modena.mirrorBase,
  yaw: modena.yawDeg * Math.PI / 180,
  pitch: modena.pitchDeg * Math.PI / 180,
});
console.log(`Mirror center: [${m.center.map(v => v.toFixed(6)).join(', ')}]`);
console.log(`Mirror normal: [${m.normal.map(v => v.toFixed(6)).join(', ')}]`);

const ec = modena.eyePoints.center;
const leftEye = [ec[0], ec[1] - modena.eyePoints.ipd/2, ec[2]];
const rightEye = [ec[0], ec[1] + modena.eyePoints.ipd/2, ec[2]];
const farPlaneX = ec[0] + modena.farDist;
const halfW = modena.reqWidth / 2;
const gzReg = modena.ground.zAtX(farPlaneX);
...
const result = fiveLineVerification({ virtualEyes, mirror: m, regEndpoints: regEps, rearWindow: modena.rearWindow });
...
assert(result.mirrorPass, 'Modena 默认角度五线 5/5 PASS');
// 逐位对照 Python 命中点 (lx, ly) — Python main.py --config data/vehicles/modena.yaml 输出
// 注意: ly 随地面坡度变化 (modena.json 用坡度地面 φ≈-0.155°, 非平地)
const expected = {
  'C→BL': { lx: -92.6, ly: 1.9 },
  'C→BR': { lx: 89.0, ly: 0.2 },
  'C→+X': { lx: -8.0, ly: 12.9 },
...
const searchResult = searchPassingAngles({
  mirrorBase: modena.mirrorBase, eyePoints: modena.eyePoints,
  farDist: modena.farDist, reqWidth: modena.reqWidth,
  ground: modena.ground, rearWindow: modena.rearWindow,
...
// ======================================================================
// 3. Kunlun 配置角度五线校核 (对照 Python git 历史: yaw=-22 pitch=8, 5/5 PASS)
// ======================================================================
console.log("\n=== 3. Kunlun 配置角度五线校核 ===\n");
const kunlun = loadVehicle('kunlun');
const mk = new Mirror({
  ...kunlun.mirrorBase,
  yaw: kunlun.yawDeg * Math.PI / 180,
  pitch: kunlun.pitchDeg * Math.PI / 180,
});
const eck = kunlun.eyePoints.center;
const farPlaneXk = eck[0] + kunlun.farDist;
const gzRegk = kunlun.ground.zAtX(farPlaneXk);
const regEpsk = [[farPlaneXk, -kunlun.reqWidth/2, gzRegk], [farPlaneXk, kunlun.reqWidth/2, gzRegk]];
const visk = [
  computeVirtualEye([eck[0], eck[1]-kunlun.eyePoints.ipd/2, eck[2]], mk),
  computeVirtualEye([eck[0], eck[1]+kunlun.eyePoints.ipd/2, eck[2]], mk),
  computeVirtualEye(eck, mk),
];
const resK = fiveLineVerification({ virtualEyes: visk, mirror: mk, regEndpoints: regEpsk, rearWindow: kunlun.rearWindow });
console.log(`五线法: ${resK.nHit}/${resK.nTot} ${resK.mirrorPass ? 'PASS' : 'FAIL'}`);
assert(resK.mirrorPass, 'Kunlun 配置角度 (yaw=-22 pitch=8) 五线 5/5 PASS');

// ======================================================================
// 4. 圆角判定 + NaN 防御 (回归: isOn(NaN,NaN) 曾返回 true → 假 PASS)
// ======================================================================
console.log("\n=== 4. 圆角判定 + NaN 防御 ===\n");
const m0 = new Mirror({ ...modena.mirrorBase, yaw: 0, pitch: 0, cornerRadius: 0 });
const m10 = new Mirror({ ...modena.mirrorBase, yaw: 0, pitch: 0, cornerRadius: 0.010 });
...
console.log("\n=== 6. pitch 优化器 ===\n");
const optMirror = {
  width: modena.mirrorBase.width, height: modena.mirrorBase.height,
  pivot: modena.mirrorBase.pivot, armOffset: modena.mirrorBase.armOffset,
  yaw: modena.yawDeg * Math.PI / 180,
};
const opt = optimizePitch({
  mirror: optMirror, eyePoints: modena.eyePoints,
  farDistance: modena.farDist, requiredWidth: modena.reqWidth,
  ground: modena.ground, // 默认 pitchRange [-5,15]
});
...
// Modena 眼点(X=3.24)在镜面(X=2.91)与后挡风(X=4.54)之间,
// 射线从眼点向后挡风(+X)不回头经过镜面 → t<0 被过滤 → 0 投影点。
// 此为 Python 算法固有行为, JS 移植一致。
// ======================================================================
console.log("\n=== 7. 后挡风投影覆盖 ===\n");
const rwProj = rearWindowProjectionOnMirror(modena.eyePoints.center, m, modena.rearWindow);
