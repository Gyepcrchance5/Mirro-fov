...
def test_integration_mirror_xiaomi():
    """真实文件: XIAOMI 内镜提取必须过闸门 (文件缺失跳过)"""
    f = TMP / 'XIAOMI-27A-INNER-MIRROR-0413a.1In_Work.stp'
    if not f.exists():
        print('  - 跳过 (无 XIAOMI STEP)')
        return
    ...
    step_verify.assert_outline_ok(pts, 'XIAOMI 镜面')
    print(f'  ✓ XIAOMI 镜面: {len(pts)} 点, 闸门通过')
...
if __name__ == '__main__':
    ...
    tests = [test_gate_detects_breaks, test_gate_detects_closure,
             ...
             test_integration_mirror_xiaomi, test_integration_rear_window]
    failed = 0
    for t in tests:
        name = t.__name__
        try:
